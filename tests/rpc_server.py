...
def process_output_thread(pipe, task_id):
    while True:
...
class MyServicer:
    async def start_worker(self, task_type, task_info):
        ...
        if task_type == TaskTypeEnum.Detect.value:
            ...
            #     error_output = output
            #     # Todo: Add error to Redis
            #     print("Error output:", error_output)
            #     msg = {"error_msg": error_output, "error_code": "-1"}
            # else:
            #     print("aaa!!!!!!!!!!!")
                # output_thread = threading.Thread(target=process_output_thread, args=(pipe, task_id), daemon=True)
                # output_thread.start()
