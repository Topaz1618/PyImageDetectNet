...
def decompress_zip(task_id, zip_file_path):
    ...
    res_path = os.path.join("upload", folder_name)
    ...
    with zipfile.ZipFile(zip_file_path, 'r') as zf:
        ...
        for fn in zf.namelist():
            ...
            with open(os.path.join("upload", right_fn), 'wb') as output_file:  # 创建并打开新文件
                with zf.open(fn, 'r') as origin_file:  # 打开原文件
                    shutil.copyfileobj(origin_file, output_file)  # 将原文件内容复制到新文件

            logger.info(f"File Decompression Completed: {right_fn}")
            update_task_info(task_id, TaskInfoKey.LOG.value, f"File Decompression Completed: {right_fn}")
